...
Glass::build(const DatasetPtr& base) {
    try {
        ...
        initializer->ep = alg_hnsw_->getEnterPoint();
        for (int i = 0; i < num_elements; ++i) {
            int level = alg_hnsw_->getLevels(i);
            initializer->levels[i] = level;
            if (level > 0) {
                initializer->lists[i].assign(level * M_, -1);
                for (int j = 1; j <= level; ++j) {
                    int* edges = (int*)alg_hnsw_->getLinklist(i, j);
...
tl::expected<std::vector<int64_t>, Error>
Glass::add(const DatasetPtr& base) {
...
Glass::knn_search_internal(const DatasetPtr& query,
                           int64_t k,
...
tl::expected<DatasetPtr, Error>
Glass::knn_search(const DatasetPtr& query,
                  int64_t k,
                  const std::string& parameters,
...
template <typename FilterType>
tl::expected<DatasetPtr, Error>
Glass::range_search_internal(const DatasetPtr& query,
                             float radius,
                             const std::string& parameters,
...
tl::expected<DatasetPtr, Error>
Glass::range_search(const DatasetPtr& query,
                    float radius,
                    const std::string& parameters,
...
Glass::serialize(std::ostream& out_stream) {
    if (GetNumElements() == 0) {
...
tl::expected<void, Error>
Glass::deserialize(const BinarySet& binary_set) {
...
tl::expected<void, Error>
Glass::deserialize(const ReaderSet& reader_set) {
    SlowTaskTimer t("hnsw deserialize");
    if (this->alg_hnsw_->getCurrentElementCount() > 0) {
        LOG_ERROR_AND_RETURNS(ErrorType::INDEX_NOT_EMPTY,
                              "failed to deserialize: index is not empty");
...
tl::expected<void, Error>
Glass::deserialize(std::istream& in_stream) {
    SlowTaskTimer t("hnsw deserialize");
    if (this->alg_hnsw_->getCurrentElementCount() > 0) {
        LOG_ERROR_AND_RETURNS(ErrorType::INDEX_NOT_EMPTY,
                              "failed to deserialize: index is not empty");
...
Glass::feedback(const DatasetPtr& query,
                int64_t k,
                const std::string& parameters,
...
tl::expected<uint32_t, Error>
Glass::feedback(const DatasetPtr& result, int64_t global_optimum_tag_id, int64_t k) {
...
tl::expected<DatasetPtr, Error>
Glass::brute_force(const DatasetPtr& query, int64_t k) {
    try {
        CHECK_ARGUMENT(k > 0, fmt::format("k({}) must be greater than 0", k));
...
void
Glass::get_vectors(const vsag::DatasetPtr& base, void** vectors_ptr, size_t* data_size_ptr) const {

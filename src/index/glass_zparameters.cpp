...
CreateGlassParameters
CreateGlassParameters::FromJson(const std::string& json_string) {
    JsonType params = JsonType::parse(json_string);
    CreateGlassParameters obj;
    ...
    // set obj.space
    CHECK_ARGUMENT(params.contains(INDEX_GLASS),
                   fmt::format("parameters must contains {}", INDEX_GLASS));
    const auto& hnsw_param_obj = params[INDEX_GLASS];

    // set obj.max_degree
    CHECK_ARGUMENT(hnsw_param_obj.contains(HNSW_PARAMETER_M),
                   fmt::format("parameters[{}] must contains {}", INDEX_GLASS, HNSW_PARAMETER_M));
    ...
    CHECK_ARGUMENT(
        ...
        fmt::format("parameters[{}] must contains {}", INDEX_GLASS, HNSW_PARAMETER_CONSTRUCTION));
...
GlassSearchParameters
GlassSearchParameters::FromJson(const std::string& json_string) {
    JsonType params = JsonType::parse(json_string);

    GlassSearchParameters obj;
    ...
    CHECK_ARGUMENT(
        params[INDEX_GLASS].contains(HNSW_PARAMETER_EF_RUNTIME),
        fmt::format("parameters[{}] must contains {}", INDEX_GLASS, HNSW_PARAMETER_EF_RUNTIME));
    obj.ef_search = params[INDEX_GLASS][HNSW_PARAMETER_EF_RUNTIME];
    ...
    if (params[INDEX_GLASS].contains(PARAMETER_USE_CONJUGATE_GRAPH_SEARCH)) {
        obj.use_conjugate_graph_search = params[INDEX_GLASS][PARAMETER_USE_CONJUGATE_GRAPH_SEARCH];
    } else {

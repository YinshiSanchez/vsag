...
public:
    ...
    HnswIndexHandler(bool is_create,
                     bool is_build,
                     bool use_static,
                     int max_degree,
                     int ef_construction,
    ...
    ~HnswIndexHandler() {
        index_ = nullptr;
        vsag::logger::debug("   after deconstruction, hnsw index addr {} : use count {}",
                            (void*)allocator_,
                            index_.use_count());
    }
    void
    set_build(bool is_build) {
        is_build_ = is_build;
    }
    bool
    is_build(bool is_build) {
        return is_build_;
    }
    int
    build_index(const vsag::DatasetPtr& base);
    int
    get_index_number();
    int
    add_index(const vsag::DatasetPtr& incremental);
    int
    knn_search(const vsag::DatasetPtr& query,
               int64_t topk,
    ...
    void
    set_index(std::shared_ptr<vsag::Index> hnsw) {
        index_ = hnsw;
    }
    vsag::Allocator*
    get_allocator() {
...
int
HnswIndexHandler::get_index_number() {
...
int
HnswIndexHandler::knn_search(const vsag::DatasetPtr& query,
                             int64_t topk,
                             const std::string& parameters,
                             const float*& dist,
                             const int64_t*& ids,
...
create_index(VectorIndexPtr& index_handler,
    ...
    if (dtype == nullptr || metric == nullptr) {
        vsag::logger::debug(
            "   null pointer addr, dtype:{}, metric:{}", (void*)dtype, (void*)metric);
        return static_cast<int>(error);
    }
    ...
    vsag::logger::debug(
        "   index type : {}, is_supported : {}", static_cast<int>(index_type), is_support);
    if (allocator == NULL) {
        vsag_allocator = NULL;
    ...
    if (is_support) {
        ...
        nlohmann::json index_parameters{
            {"dtype", dtype}, {"metric_type", metric}, {"dim", dim}, {"hnsw", hnsw_parameters}};
        if (auto index =
                vsag::Factory::CreateIndex("hnsw", index_parameters.dump(), vsag_allocator);
            index.has_value()) {
            hnsw = index.value();
            ...
            vsag::logger::debug(
                "   success to create hnsw index , index parameter:{}, allocator addr:{}",
                index_parameters.dump(),
                (void*)vsag_allocator);
            return 0;
        } else {
            error = index.error().type;
            vsag::logger::debug("   fail to create hnsw index , index parameter:{}",
                                index_parameters.dump());
        }
    } else {
        error = vsag::ErrorType::UNSUPPORTED_INDEX;
        vsag::logger::debug("   fail to create hnsw index , index type not supported:{}",
...
build_index(VectorIndexPtr& index_handler, float* vector_list, int64_t* ids, int dim, int size) {
    ...
    dataset->Dim(dim)->NumElements(size)->Ids(ids)->Float32Vectors(vector_list)->Owner(false);
...
add_index(VectorIndexPtr& index_handler, float* vector, int64_t* ids, int dim, int size) {
    ...
    incremental->Dim(dim)->NumElements(size)->Ids(ids)->Float32Vectors(vector)->Owner(false);
...
knn_search(VectorIndexPtr& index_handler,
    ...
    roaring::api::roaring64_bitmap_t* bitmap =
        static_cast<roaring::api::roaring64_bitmap_t*>(invalid);
...
fdeserialize(VectorIndexPtr& index_handler, std::istream& in_stream) {
    ...
    vsag::logger::debug("   Deserilize hnsw index , index parameter:{}, allocator addr:{}",
                        index_parameters.dump(),
                        (void*)hnsw->get_allocator());
    if (auto index =
            vsag::Factory::CreateIndex("hnsw", index_parameters.dump(), hnsw->get_allocator());
        index.has_value()) {
...
delete_index(VectorIndexPtr& index_handler) {
    ...
    vsag::logger::debug("   delete index handler addr {} : hnsw index use count {}",
                        ...
                        static_cast<HnswIndexHandler*>(index_handler)->get_index().use_count());
...
extern void
set_logger_c(void* logger_ptr) {
    set_logger(logger_ptr);
}

extern void
set_block_size_limit_c(uint64_t size) {
    set_block_size_limit(size);
}

extern bool
is_supported_index_c(IndexType index_type) {
    return is_supported_index(index_type);
}

extern int
create_index_c(VectorIndexPtr& index_handler,
               IndexType index_type,
               const char* dtype,
               const char* metric,
               int dim,
               ...
               int ef_search,
               void* allocator) {
    return create_index(index_handler,
                        index_type,
                        dtype,
                        metric,
                        dim,
...
extern int
build_index_c(VectorIndexPtr& index_handler, float* vector_list, int64_t* ids, int dim, int size) {
    return build_index(index_handler, vector_list, ids, dim, size);
}

extern int
add_index_c(VectorIndexPtr& index_handler, float* vector, int64_t* ids, int dim, int size) {
    return add_index(index_handler, vector, ids, dim, size);
}

extern int
get_index_number_c(VectorIndexPtr& index_handler, int64_t& size) {
    return get_index_number(index_handler, size);
}

extern int
knn_search_c(VectorIndexPtr& index_handler,
             float* query_vector,
             int dim,
             int64_t topk,
             const float*& dist,
             ...
             void* invalid) {
    return knn_search(
        index_handler, query_vector, dim, topk, dist, ids, result_size, ef_search, invalid);
}

extern int
serialize_c(VectorIndexPtr& index_handler, const std::string dir) {
    return serialize(index_handler, dir);
}

extern int
fserialize_c(VectorIndexPtr& index_handler, std::ostream& out_stream) {
    return fserialize(index_handler, out_stream);
}

extern int
delete_index_c(VectorIndexPtr& index_handler) {
    return delete_index(index_handler);
}
extern int
fdeserialize_c(VectorIndexPtr& index_handler, std::istream& in_stream) {
    return fdeserialize(index_handler, in_stream);
}

extern int
deserialize_bin_c(VectorIndexPtr& index_handler, const std::string dir) {
